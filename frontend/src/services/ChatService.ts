...
class ChatService extends BaseService {
  ...
  async askQuestion(question: string): Promise<ChatResponseInterface> {
    return (await this.makeRequest(ChatService.BASE_URL, 'post', {
      question,
  ...
  async updateChatName(
    ...
    return (await this.makeRequest(`${ChatService.BASE_URL}/${chatId}`, 'put', {
      new_name: newName,
    })) as { success: boolean; message: string }
  }

  async deleteChat(chatId: number): Promise<{ success: boolean; message: string }> {
    return (await this.makeRequest(`${ChatService.BASE_URL}/${chatId}`, 'delete')) as {
      success: boolean
      message: string
    }
  }
  ...
  async clearChatMessages(chatId: number): Promise<{ success: boolean; message: string }> {
    return (await this.makeRequest(`${ChatService.BASE_URL}/${chatId}/clear`, 'delete')) as {
      success: boolean
      message: string
    }
  }
