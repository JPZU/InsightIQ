...
async def upload_csv(file: UploadFile = File(...), table_name: str = Form(...), file_manager_service: FileManagerService = Depends(get_file_manager_service)):
...
async def upload_excel(file: UploadFile = File(...), table_name: str = Form(...), sheet_name: str = Form("0"), file_manager_service: FileManagerService = Depends(get_file_manager_service)):
    # Save the uploaded file to a temporary location
    file_location = FileManager.save_upload_file(file, table_name)
